...
export function CommitHero({ onNewCommit }: CommitHeroProps) {
        ...
        switch (message.type) {
          case 'gitStatsResponse':
            if (message.data) {
              ...
              // 根据Git统计数据更新XP
              const newXP = message.data.totalCommits * 10 + message.data.currentStreak * 5;
              setCurrentXP(newXP);
            }
            break;
          case 'updateTrackingStatus':
            setGitStats(prev => ({ ...prev, isTracking: message.isTracking }));
            break;
        }
      };
    }
  ...
  const handleStartTracking = () => {
    if (window.vscodeAPI) {
      window.vscodeAPI.postMessage({ type: 'startTracking' });
    }
  };

  const handleStopTracking = () => {
    if (window.vscodeAPI) {
      window.vscodeAPI.postMessage({ type: 'stopTracking' });
    }
  };

  const handleAddMockCommit = () => {
    if (window.vscodeAPI) {
      window.vscodeAPI.postMessage({ type: 'addMockCommit' });
    }
  };

  const handleClearData = () => {
    if (window.vscodeAPI) {
      window.vscodeAPI.postMessage({ type: 'clearData' });
    }
  };
    ...
    <Card className="w-full h-[520px] bg-[#252a3a] border-[#3a4051] p-6 text-white rounded-2xl">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          <span className="text-slate-300 text-sm">
            {gitStats.isTracking ? '正在追踪' : '未开始追踪'}
          </span>
          <div className="flex gap-2">
            <Button
              onClick={handleStartTracking}
              disabled={gitStats.isTracking}
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xl font-semibold text-white">{gitStats.currentStreak} days</span>
        </div>
      </div>
