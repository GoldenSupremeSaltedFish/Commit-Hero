...
export function CommitHero({ onNewCommit, gitStats, isTracking }: CommitHeroProps) {
  // Calculate XP based on real data
  const currentXP = gitStats
    ? gitStats.totalCommits * 10 + gitStats.totalLinesAdded * 0.1 + gitStats.streakDays * 5
    : 0;
  const targetXP = 300;
  const streak = gitStats?.streakDays || 0;
  ...
  return (
    <Card className='w-full h-[520px] bg-[#252a3a] border-[#3a4051] p-6 text-white rounded-2xl'>
      ...
      <div className='space-y-2'>
        ...
        <p className='text-slate-300 text-sm'>Next Goal: Reach {targetXP} XP for 'Code Ninja'</p>
      </div>
    </Card>
  );
}
