import { Bug, Calendar, Code, Share, Trophy } from 'lucide-react';
...
export function AchievementModal({ onClose, achievement }: AchievementModalProps) {
  const [isFlipped, setIsFlipped] = useState(false);

  // Use real achievement data or fallback to default
  const displayAchievement = achievement || {
    id: 'bug-buster',
    name: 'Bug Buster',
    description: 'Fixed 10 bugs in a day!',
    icon: '🐛',
          ...
          <div className='w-full h-full bg-gradient-to-br from-amber-400 via-yellow-500 to-orange-500 rounded-full p-4 shadow-lg flex flex-col items-center justify-center text-center'>
            <div className='bg-white/20 backdrop-blur-sm rounded-full p-6 w-full h-full flex flex-col items-center justify-center'>
              ...
              <div className='text-white text-xs space-y-1'>
                ...
                <div className='text-yellow-200 font-semibold text-sm mt-2'>Level 3</div>
                <div className='text-white/80 text-xs'>Epic Debugging!</div>
              </div>
            </div>
          </div>
        </motion.div>
    ...
    <Card className='w-full h-[520px] bg-[#252a3a] border-[#3a4051] p-6 text-white rounded-2xl'>
      <div className='flex flex-col h-full'>
        ...
        <div className='flex-1 flex flex-col items-center justify-center space-y-5'>
          ...
          >
            <h3 className='text-3xl font-bold text-yellow-400'>{displayAchievement.name}</h3>
            <p className='text-slate-300 text-lg'>
              {isFlipped ? 'Congratulations on your achievement!' : displayAchievement.description}
            </p>
